...
def venues():
    ...
    for venue in venue_list:
        updated = False
        for area in data:
            if area.get('city') == venue.city and area.get('state') == venue.state:
                area.get('venues').append({
                    'id': venue.id,
                    'name': venue.name,
                    'num_upcomming_shows': len(venue.shows)
                })
                updated = True
                break
        if not updated:
...
def search_venues():
    ...
    for venue in venues:
        if venue.name.lower().find(search_str) != -1:
            count += 1
            data.append({
                'id': venue.id,
                'name': venue.name,
